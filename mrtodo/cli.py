...
import typer
from mrtodo import ERRORS, __app_name__, __version__, config, database, mrtodo
...
def get_todoer() -> mrtodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():  # Check if config file exits
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            "Config file not found. Please, run \"mrtodo init\"",
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)  # Exit code of 1 to signal an error
    if db_path.exists():
        return mrtodo.Todoer(db_path)
    else:
        typer.secho(
            "Database not found. Please, run \"mrtodo init\"",
            fg=typer.colors.RED
        )
        raise typer.Exit(1)  # Exit code of 1 to signal an error
...
@app.callback()  # Callbacks allow CLI parameters for the main CLI application
def main(
